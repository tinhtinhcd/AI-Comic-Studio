        ...
        <div className="fixed inset-x-4 top-20 md:absolute md:inset-auto md:right-6 md:top-20 md:w-96 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col z-50 animate-in fade-in slide-in-from-bottom-4 md:slide-in-from-right-4 ring-1 ring-gray-900/5 dark:ring-black/20 max-h-[calc(100vh-140px)] md:max-h-[600px]">
                        ...
                        >
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <span className={`text-xs font-medium block leading-relaxed break-words ${task.isCompleted ? 'text-gray-400 dark:text-gray-500 line-through' : 'text-gray-700 dark:text-gray-200'}`}>
                                        {task.description}
                                    </span>
                                </div>
                                {task.type === 'USER' && (
            ...
            <div className="p-3 border-t border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-b-2xl space-y-2 shrink-0">
                <input 
                    ...
                    className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-xs outline-none focus:border-indigo-300 dark:focus:border-indigo-500 transition-colors focus:bg-white dark:focus:bg-gray-900 text-gray-700 dark:text-gray-200 placeholder-gray-400"
                />
                ...
                <div className="flex gap-2">
                    ...
                    <div className="flex bg-gray-50 dark:bg-gray-900 rounded-lg p-1 border border-gray-200 dark:border-gray-700">
                        {(['LOW', 'MEDIUM', 'HIGH'] as TaskPriority[]).map(p => (
                            <button
                                ...
                                className={`p-1.5 rounded-md transition-all ${newPriority === p ? 'bg-white dark:bg-gray-700 shadow-sm' : 'hover:bg-gray-200 dark:hover:bg-gray-800'}`}
                                title={p}
                            >
                                <Flag className={`w-3 h-3 ${getPriorityIconColor(p)} ${newPriority === p ? 'fill-current' : ''}`}/>
                            </button>
                        ))}
                    </div>
                    ...
                    <button 
                        ...
                        className="bg-gray-800 dark:bg-gray-700 hover:bg-gray-900 dark:hover:bg-gray-600 disabled:opacity-50 text-white px-3 py-2 rounded-lg transition-colors shadow-sm flex items-center justify-center"
                    >
