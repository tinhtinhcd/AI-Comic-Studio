...
export const ManagerView: React.FC<ManagerViewProps> = ({ 
    ...
    if (activeTab === 'TEAM') {
        const teamMembers = Object.values(AGENTS);
        return (
            <div className="flex flex-col h-full pb-8">
                {renderTabs()}
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700 shadow-sm flex-1 overflow-y-auto">
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            <h3 className="font-bold text-xl text-gray-800 dark:text-gray-100">{t('team.title')}</h3>
                            <p className="text-sm text-gray-500">{t('team.desc')}</p>
                        </div>
                        <div className="px-4 py-2 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 rounded-full text-xs font-bold border border-emerald-200 dark:border-emerald-800 flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {teamMembers.map((agent) => (
                            <div key={agent.id} className="relative group p-6 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-indigo-300 dark:hover:border-indigo-500 transition-all hover:shadow-lg">
                                <div className={`absolute top-0 left-0 right-0 h-1 rounded-t-2xl ${agent.color.replace('text-', 'bg-')}`}></div>
                                <div className="flex items-start justify-between mb-4">
                                    <div className="w-14 h-14 rounded-full border-2 border-white dark:border-gray-600 shadow-md overflow-hidden bg-gray-100 dark:bg-gray-700">
                                    ...
                                    <span className="text-[10px] uppercase font-bold text-gray-400 border border-gray-200 dark:border-gray-700 px-2 py-1 rounded-lg">
                                        {t(agent.department)}
                                    </span>
                                </div>
                                <h4 className="font-bold text-gray-900 dark:text-white text-lg mb-1">{t(agent.name)}</h4>
                                <p className="text-xs font-mono text-indigo-600 dark:text-indigo-400 mb-3 uppercase tracking-wide">{agent.id.replace('_', ' ')}</p>
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
                                    ...
                                    <button disabled className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-3 py-1.5 rounded-lg font-bold opacity-50 cursor-not-allowed">
                                        Message
                                    </button>
                                </div>
                            </div>
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-[10px] opacity-70 flex items-center gap-1">
                                                            ...
                                                            Added: {new Date(k.timestamp).toLocaleString()} ({k.provider})
                                                        </p>
                                                    </div>
                                                </div>
