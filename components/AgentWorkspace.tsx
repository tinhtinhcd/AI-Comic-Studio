...
const AgentWorkspace: React.FC<AgentWorkspaceProps> = ({ role, project, updateProject }) => {
      ...
      try {
          ...
          const chars = result.panels.flatMap(p => p.charactersInvolved).reduce((acc: Character[], name) => {
            if (!acc.find(c => c.name === name)) {
                ...
                // For this simplistic app, we assume new characters unless name matches exactly
                const existing = project.characters.find(c => c.name === name);
                if (existing) {
                    // Reuse existing locked design if available
                    acc.push(existing); 
                } else {
  ...
  const handleApproveScriptAndVisualize = async () => {
    ...
    if (isLongFormat) {
        ...
        if (unlockedChars.length > 0) {
            // Warn but allow (or could block completely)
    ...
    try {
        ...
        for (let i = 0; i < newChars.length; i++) {
             ...
             if (newChars[i].isLocked && newChars[i].imageUrl) {
                 continue;
             }

             // Generate (If Long Format, generates Reference Sheet)
             ...
             newChars[i] = { 
                 ...
                 isLocked: isLongFormat ? true : false // Auto-lock for convenience, but user can unlock
            };
             updateProject({ characters: [...newChars] }); 
        }
        ...
        for (let i = 0; i < newPanels.length; i++) {
             if (!newPanels[i].imageUrl) {
                 newPanels[i].isGenerating = true;
                 updateProject({ panels: [...newPanels] });

                 // Pass characters for consistency (GeminiService handles logic of using Locked images)
                 const imageUrl = await GeminiService.generatePanelImage(newPanels[i], project.style, newChars);
                 
                 newPanels[i] = { ...newPanels[i], imageUrl, isGenerating: false };
                 updateProject({ panels: [...newPanels] });
             }
        }
  ...
  if (role === AgentRole.PROJECT_MANAGER) {
    return (
      <div className="h-full flex flex-col max-w-7xl mx-auto w-full pt-8 px-6 relative">
        {/* Rejection Modal */}
        {showRejectModal && (
  ...
  // --- CHARACTER DESIGNER (UPDATED FOR LOCKING) ---
  if (role === AgentRole.CHARACTER_DESIGNER) {
      ...
      return (
        <div className="h-full flex flex-col p-8 max-w-7xl mx-auto w-full overflow-y-auto">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-12">
                 {project.characters.map((char, i) => (
                     <div key={char.id} className={`bg-zinc-900 border rounded-2xl p-4 flex gap-4 transition-all ${char.isLocked ? 'border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.1)]' : 'border-zinc-800'}`}>
                         <div className="w-32 h-40 bg-zinc-950 rounded-xl overflow-hidden shrink-0 relative">
                             {char.imageUrl ? (
                         ...
                         <div className="flex-1 flex flex-col">
                             ...
                             <p className="text-xs text-zinc-400 line-clamp-3 mb-auto">{char.description}</p>
  ...
  if (role === AgentRole.SCRIPTWRITER) {
    return (
      <div className="h-full flex flex-col p-8 max-w-5xl mx-auto w-full overflow-y-auto">
         <div className="flex items-center gap-6 mb-8">
  ...
  if (role === AgentRole.CINEMATOGRAPHER) {
      return (
        <div className="h-full flex flex-col p-8 max-w-6xl mx-auto w-full overflow-y-auto">
            <div className="flex items-center gap-6 mb-8">
  ...
  if (role === AgentRole.PUBLISHER) {
      return (
          <div className="h-full flex items-center justify-center p-8 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-zinc-900 to-zinc-950">
