...
const AgentWorkspace: React.FC<AgentWorkspaceProps> = ({ role, project, updateProject, onAgentChange, uiLanguage, currentUser, onUpdateUser }) => {
  ...
  // New state for switching between Manager and Profile view
  ...
  // Inject current User ID into the hook
  const { 
      ...
      { ...project, ownerId: currentUser.id }, // Force ownerId
      updateProject, 
      uiLanguage
  );
  ...
  // --- AUTO-COMPLETE TASKS LOGIC (unchanged) ---
  // ... (keeping existing logic)

  // ... (keeping handleImportManuscript, checkApiKeyRequirement, etc.)
  ...
  // Helper to map Stage -> Agent for transitions (unchanged)
  ...
  // ... (All existing logic for handleStartResearch, chat, etc. remains the same)
  // ... (Included just the required logic updates for brevity, assuming full context is preserved in final output)
  ...
  const handleResearchChatSend = async () => {
      ...
      try {
          ...
          while ((match = commandRegex.exec(aiResponseText)) !== null) {
              ...
              } catch (err) { console.error("Failed to parse Agent Command:", err); }
          }
  ...
  const handleGenerateFinalScript = async () => { setScriptStep('WRITING'); const targetChapter = project.currentChapter || 1; let dynamicPanelCount = 25; if (project.storyFormat === 'EPISODIC') dynamicPanelCount = 60; if (project.storyFormat === 'LONG_SERIES') dynamicPanelCount = 45; addLog(AgentRole.SCRIPTWRITER, `Drafting Chapter ${targetChapter} (Target: ~${dynamicPanelCount} panels)...`, 'info'); let chapterSummary = ""; if (project.marketAnalysis?.chapterOutlines) { const outline = project.marketAnalysis.chapterOutlines.find(c => c.chapterNumber === targetChapter); if (outline) chapterSummary = outline.summary; } try { if (isLongFormat && !project.seriesBible) { const bible = await GeminiService.generateSeriesBible(project.theme, project.style, project.masterLanguage, project.modelTier || 'STANDARD'); updateProject({ seriesBible: bible }); } const setting = project.marketAnalysis?.worldSetting; const result = await GeminiService.generateScript(project.theme, project.marketAnalysis ? project.marketAnalysis.visualStyle : project.style, project.masterLanguage, project.storyFormat, project.seriesBible, project.modelTier || 'STANDARD', project.storyConcept, project.characters, chapterSummary, targetChapter, project.originalScript, setting, dynamicPanelCount); updateProject({ title: result.title, panels: result.panels, workflowStage: WorkflowStage.CENSORING_SCRIPT }); addLog(AgentRole.SCRIPTWRITER, `Script Draft Complete (${result.panels.length} panels). Auto-starting Compliance Scan...`, 'success'); addLog(AgentRole.CENSOR, "Scanning script content...", 'info'); const fullText = result.panels.map((p: any) => p.description + " " + p.dialogue).join("\n"); const censorResult = await GeminiService.censorContent(fullText, 'SCRIPT'); updateProject({ censorReport: censorResult.report, isCensored: !censorResult.passed }); if (censorResult.passed) { addLog(AgentRole.CENSOR, "Content passed safety checks. Auto-advancing to Character Design.", 'success'); updateProject({ workflowStage: WorkflowStage.DESIGNING_CHARACTERS }); setTimeout(() => onAgentChange(AgentRole.CHARACTER_DESIGNER), 1000); } else { addLog(AgentRole.CENSOR, "Safety issues detected. Review required.", 'warning'); setTimeout(() => onAgentChange(AgentRole.CENSOR), 1000); } } catch (e: any) { addLog(AgentRole.SCRIPTWRITER, "Script generation failed: " + e.message, 'error'); throw e; } };
  
  // Handlers for exporting and importing script
  ...
  // Handlers for approval and visualization
  ...
  const handleGenerateAllCharacters = async (selectedStyle: string) => { if (!project.characters || project.characters.length === 0) { (window as any).alert("No characters found. Please ensure the Cast has been generated in the Scriptwriter step."); return; } setLoading(true); updateProject({ style: selectedStyle }); const currentImageModel = project.imageModel || 'gemini-2.5-flash-image'; addLog(AgentRole.CHARACTER_DESIGNER, `Starting batch generation for ${project.characters.length} characters. Style: ${selectedStyle}. Engine: ${currentImageModel}`, 'info'); const charsStart = project.characters.map(c => { if (c.isLocked && c.imageUrl) return c; return { ...c, isGenerating: true, error: undefined }; }); updateProject({ characters: [...charsStart] }); await delay(100); try { await checkApiKeyRequirement(); addLog(AgentRole.CHARACTER_DESIGNER, "Generating Art Style Guide...", 'info'); const worldSetting = project.seriesBible?.worldSetting || project.marketAnalysis?.worldSetting || ""; let styleGuide = project.artStyleGuide; try { if (!styleGuide || !styleGuide.includes(selectedStyle)) { styleGuide = await GeminiService.generateArtStyleGuide(selectedStyle, worldSetting, project.masterLanguage, project.modelTier); updateProject({ artStyleGuide: styleGuide }); addLog(AgentRole.CHARACTER_DESIGNER, "New Style Guide enforced.", 'success'); } } catch (err) { console.warn("Style guide generation failed, falling back to raw style name."); styleGuide = `Style: ${selectedStyle}. Setting: ${worldSetting}`; } const workingChars = [...charsStart]; for (let i = 0; i < workingChars.length; i++) { if (workingChars[i].isLocked && workingChars[i].imageUrl) continue; addLog(AgentRole.CHARACTER_DESIGNER, `Generating ${workingChars[i].name}...`, 'info'); try { const result = await GeminiService.generateCharacterDesign(workingChars[i].name, styleGuide!, workingChars[i].description, worldSetting, project.modelTier || 'STANDARD', currentImageModel); workingChars[i] = { ...workingChars[i], imageUrl: result.imageUrl, description: result.description, isGenerating: false, error: undefined, variants: [...(workingChars[i].variants || []), { id: crypto.randomUUID(), imageUrl: result.imageUrl, style: selectedStyle, timestamp: Date.now() }] }; } catch (e: any) { workingChars[i] = { ...workingChars[i], isGenerating: false, error: e.message }; addLog(AgentRole.CHARACTER_DESIGNER, `Failed: ${workingChars[i].name}. ${e.message}`, 'error'); } updateProject({ characters: [...workingChars] }); await delay(1500); } addLog(AgentRole.CHARACTER_DESIGNER, "Batch generation complete.", 'success'); } catch (e: any) { addLog(AgentRole.CHARACTER_DESIGNER, `Process error: ${e.message}`, 'error'); const charsReset = project.characters.map(c => ({ ...c, isGenerating: false })); updateProject({ characters: charsReset }); } finally { setLoading(false); } };
  ...
  const handleStartPanelGeneration = async (selectedStyle: string) => { setLoading(true); await checkApiKeyRequirement(); try { const currentImageModel = project.imageModel || 'gemini-2.5-flash-image'; addLog(AgentRole.PANEL_ARTIST, `Drawing ${project.panels.length} panels in style: ${selectedStyle} (Engine: ${currentImageModel})...`, 'info'); updateProject({ style: selectedStyle }); const worldSetting = project.seriesBible?.worldSetting || project.marketAnalysis?.worldSetting || ""; let styleGuide = project.artStyleGuide; if (!styleGuide || !styleGuide.includes(selectedStyle)) { try { styleGuide = await GeminiService.generateArtStyleGuide(selectedStyle, worldSetting, project.masterLanguage, project.modelTier); updateProject({ artStyleGuide: styleGuide }); } catch (e) { styleGuide = `Style: ${selectedStyle}`; } } let updatedPanels = [...project.panels]; for (let i = 0; i < updatedPanels.length; i++) { if (!updatedPanels[i].imageUrl) { updatedPanels[i] = { ...updatedPanels[i], isGenerating: true }; updateProject({ panels: [...updatedPanels] }); try { const imageUrl = await GeminiService.generatePanelImage(updatedPanels[i], styleGuide!, project.characters, worldSetting, project.modelTier || 'STANDARD', currentImageModel); updatedPanels[i] = { ...updatedPanels[i], imageUrl, isGenerating: false }; } catch (error) { updatedPanels[i] = { ...updatedPanels[i], isGenerating: false }; } updateProject({ panels: [...updatedPanels] }); } } addLog(AgentRole.PANEL_ARTIST, "Panels ready.", 'success'); } finally { setLoading(false); } };
  ...
  const handleRegenerateSinglePanel = async (panel: ComicPanel, index: number) => { await checkApiKeyRequirement(); const panelsBefore = [...project.panels]; panelsBefore[index] = { ...panelsBefore[index], isGenerating: true }; updateProject({ panels: panelsBefore }); setRegeneratingPanelId(panel.id); try { const worldSetting = project.seriesBible?.worldSetting || project.marketAnalysis?.worldSetting || ""; const styleGuide = project.artStyleGuide || `Style: ${project.style}`; const currentImageModel = project.imageModel || 'gemini-2.5-flash-image'; const imageUrl = await GeminiService.generatePanelImage(panel, styleGuide, project.characters, worldSetting, project.modelTier || 'STANDARD', currentImageModel); const newPanels = [...project.panels]; newPanels[index] = { ...newPanels[index], imageUrl, isGenerating: false }; updateProject({ panels: newPanels }); } catch (e) { const newPanels = [...project.panels]; newPanels[index] = { ...newPanels[index], isGenerating: false }; updateProject({ panels: newPanels }); } finally { setRegeneratingPanelId(null); } };
  const handleRegenerateSingleCharacter = async (char: Character, index: number, specificStyle?: string) => { await checkApiKeyRequirement(); const charsStart = [...project.characters]; charsStart[index] = { ...charsStart[index], isGenerating: true, error: undefined }; updateProject({ characters: charsStart }); const styleToUse = specificStyle || project.style; const currentImageModel = project.imageModel || 'gemini-2.5-flash-image'; try { const worldSetting = project.seriesBible?.worldSetting || project.marketAnalysis?.worldSetting || ""; let styleGuide = project.artStyleGuide; if (specificStyle && (!styleGuide || !styleGuide.includes(specificStyle))) { styleGuide = `Style: ${specificStyle}. Setting: ${worldSetting}`; } else if (!styleGuide) { styleGuide = `Style: ${styleToUse}`; } const result = await GeminiService.generateCharacterDesign(char.name, styleGuide, char.description, worldSetting, project.modelTier || 'STANDARD', currentImageModel); let consistencyStatus: 'PASS' | 'FAIL' = 'PASS'; let consistencyReport = ''; try { const check = await GeminiService.analyzeCharacterConsistency(result.imageUrl, styleToUse, char.name, project.modelTier || 'STANDARD'); consistencyStatus = check.isConsistent ? 'PASS' : 'FAIL'; consistencyReport = check.critique; } catch (e) {} const newVariant: CharacterVariant = { id: crypto.randomUUID(), imageUrl: result.imageUrl, style: styleToUse, timestamp: Date.now() }; const charsDone = [...projectRef.current.characters]; charsDone[index] = { ...charsDone[index], imageUrl: result.imageUrl, variants: [...(charsDone[index].variants || []), newVariant], description: result.description, isGenerating: false, error: undefined, consistencyStatus, consistencyReport }; updateProject({ characters: charsDone }); } catch (e: any) { const charsFail = [...project.characters]; charsFail[index] = { ...charsFail[index], isGenerating: false, error: e.message }; updateProject({ characters: charsFail }); addLog(AgentRole.CHARACTER_DESIGNER, `Failed to regenerate ${char.name}: ${e.message}`, 'error'); } };
  ...
  const handleCompleteChapterAndNext = async () => { if(!(window as any).confirm("Archive current panels and start next chapter?")) return; setLoading(true); try { const summary = await GeminiService.summarizeChapter(project.panels, project.modelTier || 'STANDARD'); const chapterData: ChapterArchive = { chapterNumber: project.currentChapter || 1, title: `Chapter ${project.currentChapter || 1}`, panels: [...project.panels], summary: summary, timestamp: Date.now() }; const nextChapter = (project.currentChapter || 1) + 1; let newTasks = project.agentTasks || []; newTasks = newTasks.map(t => { if (t.targetChapter === project.currentChapter && t.type === 'SYSTEM') return { ...t, isCompleted: true }; return t; }); if (isLongFormat) { handleBreakdownChapterTasks(nextChapter, project); } const updatedProject: Partial<ComicProject> = { completedChapters: [...(project.completedChapters || []), chapterData], panels: [], currentChapter: nextChapter, workflowStage: WorkflowStage.SCRIPTING, agentTasks: newTasks }; updateProject(updatedProject); addLog(AgentRole.PROJECT_MANAGER, `Chapter ${chapterData.chapterNumber} Finished. Proceeding to Chapter ${nextChapter}.`, 'success'); StorageService.saveWorkInProgress({ ...project, ...updatedProject } as ComicProject); } catch(e) { addLog(AgentRole.PROJECT_MANAGER, "Failed to archive chapter.", 'error'); } finally { setLoading(false); } };
  ...
  // ... (Rest of Agent Views remain identical)
